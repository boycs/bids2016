...
public class PagerSystemTradeBidSystem extends Fragment {
	...
	private void initSetData() {
		// Log.v("initSetData", ""+PagerSystemTrade.contentGetEma);
		try {

			if (FragmentChangeActivity.contentGetSystemTradeEma != null) {
				if (statBuySell == 0) {
					jsaBuySell = FragmentChangeActivity.contentGetSystemTradeEma
							.getJSONArray("buy");
				...
				for (int i = 0; i < jsaBuySell.length(); i++) {
					...
					TextView tv_volume = (TextView)viewDetail.findViewById(R.id.tv_volume);
					TextView tv_value = (TextView)viewDetail.findViewById(R.id.tv_value);
