...
public class DialogAddPortfolio {

	private static Dialog dialog;
	private static Context context;
	private static Dialog dialogLoading;

	public DialogAddPortfolio(Context context2) {
	...
	// ============== Load initLoadData =========================
	public static JSONArray contentGetDataSelect = null;
	...
	public static class loadAll extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if (FragmentChangeActivity.selectSearchPortStock) {
				url_GetData = SplashScreen.url_bidschart
						...
						+ FragmentChangeActivity.strSearchPortName
						+ "&timestamp=" + timestamp;
			} else {
				url_GetData = SplashScreen.url_bidschart
			...
			Log.v("dialog url_ getPortfolio add", "" + url_GetData);
				...
				if (jsonGetData != null) {
					try {
						if ((jsonGetData.getString("status")).equals("ok")) {
							if (FragmentChangeActivity.selectSearchPortStock) {
								contentGetDataSelect = jsonGetData
							...
							// Log.v("contentGetDataSelect", "" +
							// contentGetDataSelect);

							if (contentGetDataSelect != null) {
	...
	public static void initSetData() {
		...
		try {
			if (contentGetDataSelect != null) {
				...
				if (FragmentChangeActivity.selectSearchPortStock) {
					name_initial = contentGetDataSelect.getJSONObject(0)
							.getString("symbol_name");
					invest_value = contentGetDataSelect.getJSONObject(0)
							.getString("last_trade");
					name_e = contentGetDataSelect.getJSONObject(0).getString(
							"symbol_fullname_eng");
					invest_change = contentGetDataSelect.getJSONObject(0)
				...
				if (!invest_value.equals("") && !invest_value.equals("null") && !invest_value.equals(null)) {
...
//				+ FunctionSymbol.setFormatAnd2Digit("" + d_invest));
	...
	public static void setRowDetail() {
		try {
			if (contentGetDataSelect != null) {
				...
				JSONObject jsoIndex = contentGetDataSelect.getJSONObject(0);
				if (FragmentChangeActivity.selectSearchPortStock) {
	...
	// listPricePeroid = new ArrayList<Double>();
	// double priceUp = dLastTrade;
	//
	// listPricePeroid.add(dLastTrade);
	// do {
	// double gap = calGapUp(priceUp);
	// priceUp = priceUp + gap;
	//
	// listPricePeroid.add(0, priceUp);
	// } while (priceUp < dCeilling);
	// double priceDown = dLastTrade;
	// do {
	// double gap = calGapDown(priceDown);
	// priceDown = priceDown - gap;
	//
	// listPricePeroid.add(priceDown);
	// } while (priceDown > dFloor);
