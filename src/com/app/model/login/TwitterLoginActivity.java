...
public class TwitterLoginActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.login_main_twitter);
		...
		Log.v("isLoggedIn", ""+isLoggedIn);
		Log.v("mSharedPreferences", ""+mSharedPreferences);
		...
		if (isLoggedIn) {
			...
			Log.v("isLoggedIn mSharedPreferences", ""+mSharedPreferences.getAll());
			// {twitter_user_name=boy, oauth_token=88905061-XM6smUXZN4cEwEuClWv9PIFFWsv5HqqK2BYeKhOk6, 
			// is_twitter_loggedin=true, oauth_token_secret=DXcJYD0w1DIA3CJ8tDEOoLbMJ0q8g3iklzkN55h0eakhE}
	...
	private void loginToTwitter() {
		...
		Log.v("loginToTwitter", ""+isLoggedIn); // true is login
		
		if (!isLoggedIn) { // ยังไม่ login
			...
			try {
				...
				Log.v("loginToTwitter requestToken", ""+requestToken);
...
//				Log.v("onActivityResult requestCode", ""+accessToken);
				...
				loginLayout.setVisibility(View.GONE);
				shareLayout.setVisibility(View.VISIBLE);
				userName.setText(TwitterLoginActivity.this.getResources()
						.getString(R.string.hello) + user);

//				SplashScreen.userModel.firstName = SplashScreen.user
//						.getFirstName();
//				SplashScreen.userModel.lastName = SplashScreen.user
//						.getLastName();
//				SplashScreen.userModel.userName = SplashScreen.user.getName();
//				SplashScreen.userModel.token = session.getAccessToken();
//				SplashScreen.userModel.email = SplashScreen.user.getProperty(
//						"email").toString();
//				SplashScreen.userModel.profileImagePath = "http://graph.facebook.com/"
//						+ SplashScreen.user.getId() + "/picture?type=normal";
				
				Log.v("twitter user", ""+user);			
	...
	class updateTwitterStatus extends AsyncTask<String, String, Void> {
		...
		protected Void doInBackground(String... args) {
			...
			try {
				...
				Log.v("Status updateTwitterStatus", response.getText());
		...
		protected void onPostExecute(Void result) {
			...
			Toast.makeText(TwitterLoginActivity.this, "Posted to Twitter!",
					Toast.LENGTH_SHORT).show();
